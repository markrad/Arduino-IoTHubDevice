#include <AzureIoTSocket_Ethernet2.h>
    ...
    case IOTHUBMESSAGE_BYTEARRAY:
      if (IOTHUB_MESSAGE_OK == (iotHubMessage.GetByteArray(&buffer, &size)))
      {
        ...
        if (size == 4 && 0 == memcmp(buffer, "kill", 4))
        {
          ...
          Serial.println("Could be some sort of test here in the future");
        }
      }
      else
      {
...
// Initialize the Ethernet board - sample uses DHCP to acquire IP address, DNS server and Gateway
void initEthernet()
{
  // Generate a random MAC address - you may need to change this
  for (int i = 1; i < sizeof(mac); i++)
  {
